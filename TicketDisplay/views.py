...
def return_custom_field_dict(ticket):
		#Constructs dict of custom fields
		custom_field_dict = {}
...
def returnTickets():
	#Returns a list of dicts. Each list element contains dict that contains ticket data
	...
	for ticket in last_3_tickets:
		...
		#Try-except block for retreiving problem description for ticket
		try:
...
def dashboard(request):
	...
	#List of created and resolved tickets and day names for last 7 days
	for i in range(7,0,-1): 
		delta = timedelta(days=i-1)
		past_date = current_date - delta

		count = get_created_on_date(past_date).count()
		day_name = html.unescape(past_date.strftime("%A")) #List of day names for given date
	...
	#List of created and resolved tickets and dates for last 30 days
	for j in range(30,0,-1):
		delta = timedelta(days=j-1)
		past_date = current_date - delta

		dates.append(past_date.strftime("%d.%m")) #List of dates
		count = get_created_on_date(past_date).count()
	...
	#List of new and and open tickets on specific day for last 7 days
	...
	#List of new and and open tickets on specific day for last 30 days
	...
	#Obtaining number of solved tickets today and best solver
	...
	if(solved_today.count() == 0):
		...
		header_data["best_today"] = max(set(solver_list), key=solver_list.count) #Obtaining best solver
		header_data["best_today_count"] = solver_list.count(header_data["best_today"])

	solver_list=[]

	#Obtaining number of solved tickets for month and best solver
	...
	header_data["best_month"] = max(set(solver_list), key=solver_list.count) #Obtaining best solver
	header_data["best_month_count"] = solver_list.count(header_data["best_month"])
	
	#	-- Employee rankings chart --

	#Rankings by number of solved and hours for last 7 days
	delta = timedelta(days=6)
	past_date = current_date - delta

	solved_week = get_solved_tickets_last_x_days(past_date)
	for ticket in solved_week:
		#Construct dictionaries of employees with number of solved tickets and working hours
	...
	rankings = {k: v for k, v in sorted(rankings.items(), reverse=True, key=lambda item: item[1])} #Sorting dict by key values
	rankings_hours = {k: v for k, v in sorted(rankings_hours.items(), reverse=True, key=lambda item: item[1])} #Sorting dict by key values
	...
	#Rankings by number of solved and hours for last 30 days
	...
	rankings_30 = {k: v for k, v in sorted(rankings_30.items(), reverse=True, key=lambda item: item[1])} #Sorting dict by key values
	rankings_hours_30 = {k: v for k, v in sorted(rankings_hours_30.items(), reverse=True, key=lambda item: item[1])} #Sorting dict by key values
	...
	for ticket in tickets:
		label = ticket.queue.name.replace('Služba za informatiku - ','') #Shortening labels
		#Construct dictionaries of tickets by category name and hours by category name for current month
	...
	for ticket in tickets:
		...
		#Construct dictionaries of tickets by category name and hours by category name for last 3 months
	...
	context = {
		...
		'category': list(distribution.keys()), #converting dict to list for chart datasets
